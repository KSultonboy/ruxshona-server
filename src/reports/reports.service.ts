...
import { ReturnSourceType, ReturnStatus, TransferStatus, TransferTargetType } from "@prisma/client";
import { PrismaService } from "../prisma/prisma.service";

type DateRange = { from?: string; to?: string };
type ReportMetric = "revenue" | "expenses" | "payments" | "transfers" | "returns" | "netProfit" | "debt";
type ReportGranularity = "day" | "week" | "month";
type ReportSegmentBy = "branch" | "shop" | "product" | "category" | "paymentMethod" | "sourceType";
...
function sumByKey<T>(items: T[], getKey: (item: T) => string | null | undefined, getLabel: (item: T) => string, getValue: (item: T) => number) {
...
function buildSeries(items: MetricItem[], range: DateRange, granularity: ReportGranularity) {
...
export class ReportsService {
    ...
    switch (metric) {
      ...
      case "transfers": {
        const transfers = await this.prisma.transfer.findMany({
          ...
          select: {
            date: true,
            items: { select: { quantity: true, product: { select: { price: true, salePrice: true } } } },
          },
        });
        return transfers.map((t) => ({ date: t.date, value: sumItems(t.items ?? []) }));
      }
      case "returns": {
        ...
        return returns.map((r) => ({ date: r.date, value: sumItems(r.items ?? []) }));
      }
      case "netProfit": {
      ...
      case "debt": {
        ...
        return [
          ...transfers.map((t) => ({ date: t.date, value: sumItems(t.items ?? []) })),
          ...returns.map((r) => ({ date: r.date, value: -sumItems(r.items ?? []) })),
  ...
  async overview(range: DateRange) {
    ...
    const [sales, expenseAgg, paymentAgg, transfers, returns] = await Promise.all([
      this.prisma.sale.findMany({
      ...
      this.prisma.transfer.findMany({
        ...
        select: { items: { select: { quantity: true, product: { select: { price: true, salePrice: true } } } } },
      }),
      this.prisma.return.findMany({
        where: {
    ...
    const transfersTotal = transfers.reduce((sum, t) => sum + sumItems(t.items ?? []), 0);
    const returnsTotal = returns.reduce((sum, r) => sum + sumItems(r.items ?? []), 0);
  ...
  async segments(params: { metric: ReportMetric; segmentBy: ReportSegmentBy } & ReportFilters) {
    ...
    switch (metric) {
      case "revenue": {
        const sales = await this.prisma.sale.findMany({
          ...
          select: {
            ...
            product: { select: { id: true, name: true, category: { select: { id: true, name: true } }, categoryId: true } },
          },
        });
        if (segmentBy === "branch") {
          return sumByKey(sales, (s) => s.branchId, (s) => s.branch?.name ?? "Unknown", (s) => s.price * s.quantity);
        }
        if (segmentBy === "product") {
          return sumByKey(sales, (s) => s.productId, (s) => s.product?.name ?? "Unknown", (s) => s.price * s.quantity);
        }
      ...
      case "expenses": {
        ...
        if (segmentBy === "category") {
          return sumByKey(expenses, (e) => e.categoryId, (e) => e.category?.name ?? "Unknown", (e) => e.amount);
        }
        if (segmentBy === "paymentMethod") {
      ...
      case "payments": {
        ...
        if (segmentBy === "branch") {
          return sumByKey(payments, (p) => p.branchId, (p) => p.branch?.name ?? "Unknown", (p) => p.amount);
        }
        if (segmentBy === "shop") {
          return sumByKey(payments, (p) => p.shopId, (p) => p.shop?.name ?? "Unknown", (p) => p.amount);
        }
        if (segmentBy === "paymentMethod") {
      ...
      case "transfers": {
        ...
        if (segmentBy === "branch") {
          return sumByKey(transfers, (t) => t.branchId, (t) => t.branch?.name ?? "Unknown", (t) => sumItems(t.items ?? []));
        }
        if (segmentBy === "shop") {
          return sumByKey(transfers, (t) => t.shopId, (t) => t.shop?.name ?? "Unknown", (t) => sumItems(t.items ?? []));
        }
        return [];
      }
      case "returns": {
        ...
        if (segmentBy === "branch") {
          return sumByKey(returns, (r) => r.branchId, (r) => r.branch?.name ?? "Unknown", (r) => sumItems(r.items ?? []));
        }
        if (segmentBy === "shop") {
          return sumByKey(returns, (r) => r.shopId, (r) => r.shop?.name ?? "Unknown", (r) => sumItems(r.items ?? []));
        }
        return [];
      }
      default:
  ...
  async exportData(type: string, filters: ReportFilters): Promise<ReportExportData> {
    switch (type) {
      case "sales": {
        const sales = await this.prisma.sale.findMany({
          where: this.saleWhere(filters),
