...
export class ReturnsService {
  ...
  async create(dto: CreateReturnDto, user: AuthUser) {
    ...
    if (dto.sourceType === ReturnSourceType.BRANCH) {
      ...
      if (dto.shopId)
        throw new BadRequestException('Shop not allowed for branch return');
    }
    if (dto.sourceType === ReturnSourceType.SHOP) {
      if (!dto.shopId) throw new BadRequestException('Shop required');
      if (dto.branchId)
        throw new BadRequestException('Branch not allowed for shop return');
    }

    if (user.role === 'SALES') {
      if (dto.sourceType !== ReturnSourceType.BRANCH) {
        throw new ForbiddenException(
          'Sales user can only create branch return',
        );
      }
      const dbUser = await this.prisma.user.findUnique({
        where: { id: user.id },
