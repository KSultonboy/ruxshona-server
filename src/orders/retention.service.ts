  ...
  async handleBirthdayCoupons() {
    ...
    for (const customer of customers) {
      try {
        ...
        await this.notifications.notifyBirthdayCoupon(
          customer.phone,
          code,
          discount,
        );
        this.logger.log(
          `Birthday coupon generated and notified for ${customer.name} (${customer.phone})`,
        );
      } catch (e) {
        this.logger.error(
          `Failed to generate birthday coupon for ${customer.name}: ${e.message}`,
        );
      }
    }
  }
