...
export class CouponsService {
    ...
    async validate(code: string, userId: string, orderTotal: number) {
        ...
        if (!coupon) throw new BadRequestException('Kupon topilmadi yoki faol emas');
        ...
        if (orderTotal < coupon.minOrder) {
            throw new BadRequestException(`Minimal buyurtma miqdori: ${coupon.minOrder.toLocaleString()} so'm`);
        }
