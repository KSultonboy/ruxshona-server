...
export class ExpenseItemsService {
    ...
    if (category.type === "SELLABLE") {
      if (!productId) {
        const categoryId =
          ...
          (await this.prisma.productCategory.findFirst({ orderBy: { name: "asc" } }))?.id;
        if (!categoryId) {
          throw new BadRequestException("Product category required for sellable expense item");
        }
        const unit = await this.prisma.unit.findFirst({ orderBy: { name: "asc" } });
        if (!unit) throw new BadRequestException("Unit required for sellable expense item");
  ...
  async update(id: string, dto: UpdateExpenseItemDto) {
    ...
    const salePrice = dto.salePrice ?? undefined;
    if (category.type === "SELLABLE") {
      if (!productId) throw new BadRequestException("Product required for sellable expense item");
      const product = await this.prisma.product.findUnique({ where: { id: productId } });
      if (!product) throw new BadRequestException("Product not found");
      if (salePrice !== undefined) {
        await this.prisma.product.update({
