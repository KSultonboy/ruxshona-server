...
export class InventoryService {
    ...
    async finalize(id: string) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!check) throw new NotFoundException('Inventory check not found');
            if (check.status !== 'PENDING') throw new Error('Inventory check already finalized or canceled');
            ...
            for (const item of check.items) {
                ...
                if (check.branchId) {
                    ...
                    await tx.branchStock.upsert({
                        where: { branchId_productId: { branchId: check.branchId, productId: item.productId } },
                        update: { quantity: { increment: diff } },
                        create: { branchId: check.branchId, productId: item.productId, quantity: item.actualQuantity },
                    });
                } else {
    ...
    async remove(id: string) {
        ...
        if (!check) throw new NotFoundException('Inventory check not found');
        if (check.status === 'COMPLETED') throw new Error('Cannot delete completed inventory check');
